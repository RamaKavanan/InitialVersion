...
class CostbookDefaultController  extends ZurmoModuleController{    
    ...
    protected function attemptToSaveModelFromPost($model, $redirectUrlParams = null, $redirect = true, $returnOnValidate = false)
    {
        ...
        if (isset($_POST[$postVariableName]))
        {
            ...
            if($model->id > 0 ) {
                $costbook = Costbook::getByCodeAndId($product_code, $model->id);
            } else {
            ...
            if ($model->save())
            {
                if($model->costofgoodssold->value != 'Assembly') {
                   ...
                   $this->redirectAfterSaveModel($model->id, array($this->getId() . '/AssemblyStep2', 'id' => $model->id));
                }        
            }
        }
        return $model;
    ...
    public function actionDetails($id){
        ...
        if($Costbook->costofgoodssold == 'Labor') {
            $Costbook->laborCost='$'.$Costbook->laborCost;
            $Costbook->burdenCost='$'.$Costbook->burdenCost;
    ...
    public function actionGetAssemblySearchData($category, $costOfGoods, $productId) {
        $searchData = Costbook::getAssemblySearch($category, $costOfGoods, $productId);              
